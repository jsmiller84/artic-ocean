...
"use strict";C3.Plugins.AJAX=class extends C3.SDKPluginBase{constructor(a){super(a)}Release(){super.Release()}};

"use strict";C3.Plugins.AJAX.Type=class extends C3.SDKTypeBase{constructor(a){super(a)}Release(){super.Release()}OnCreate(){}};

"use strict";C3.Plugins.AJAX.Instance=class extends C3.SDKInstanceBase{constructor(a){if(super(a),this._lastData="",this._curTag="",this._progress=0,this._timeout=-1,this._nextRequestHeaders=new Map,this._nextReponseBinaryData=null,this._nextRequestOverrideMimeType="",this._nwjsFs=null,this._nwjsPath=null,this._nwjsAppFolder=null,this._isNWjs="nwjs"===this._runtime.GetExportType(),this._isNWjs){this._nwjsFs=require("fs"),this._nwjsPath=require("path");const a=self["process"]||nw["process"];this._nwjsAppFolder=this._nwjsPath["dirname"](a["execPath"])+"\\"}}Release(){super.Release()}async _TriggerError(a,b,c){console.error(`[Construct 3] AJAX request to '${b}' (tag '${a}') failed: `,c),this._curTag=a,await this.TriggerAsync(C3.Plugins.AJAX.Cnds.OnAnyError),await this.TriggerAsync(C3.Plugins.AJAX.Cnds.OnError)}async _TriggerComplete(a){this._curTag=a,await this.TriggerAsync(C3.Plugins.AJAX.Cnds.OnAnyComplete),await this.TriggerAsync(C3.Plugins.AJAX.Cnds.OnComplete)}async _OnProgress(a,b){b["lengthComputable"]&&(this._progress=b["loaded"]/b["total"],this._curTag=a,await this.TriggerAsync(C3.Plugins.AJAX.Cnds.OnProgress))}_OnError(a,b,c){if(!this._isNWjs)return void this._TriggerError(a,b,c);const d=this._nwjsFs,e=this._nwjsAppFolder+b;d["existsSync"](e)?d["readFile"](e,{"encoding":"utf8"},(c,d)=>{c?this._TriggerError(a,b,c):(this._lastData=d.replace(/\r\n/g,"\n"),this._TriggerComplete(a))}):this._TriggerError(a,b,c)}async _DoCordovaRequest(a,b){const c=this._runtime.GetAssetManager(),d=this._nextReponseBinaryData;this._nextReponseBinaryData=null;try{if(d){const e=await c.CordovaFetchLocalFileAsArrayBuffer(b);d.SetArrayBufferTransfer(e),this._lastData="",this._TriggerComplete(a)}else{const d=await c.CordovaFetchLocalFileAsText(b);this._lastData=d.replace(/\r\n/g,"\n"),this._TriggerComplete(a)}}catch(c){this._TriggerError(a,b,c)}}_DoRequest(a,b,c,d){return new Promise((e)=>{const f=(c)=>{this._OnError(a,b,c),e()},g=this._nextReponseBinaryData;this._nextReponseBinaryData=null;try{const h=new XMLHttpRequest;h.onreadystatechange=()=>{if(4===h.readyState){if(this._lastData=g?"":(h.responseText||"").replace(/\r\n/g,"\n"),400<=h.status)this._TriggerError(a,b,h.status+h.statusText);else{const b=this._lastData.length||g&&h.response instanceof ArrayBuffer;this._isNWjs&&!b||!this._isNWjs&&0===h.status&&!b||(g&&g.SetArrayBufferTransfer(h.response),this._TriggerComplete(a))}e()}},h.onerror=f,h.ontimeout=f,h.onabort=f,h["onprogress"]=(b)=>this._OnProgress(a,b),h.open(c,b),0<=this._timeout&&"undefined"!=typeof h["timeout"]&&(h["timeout"]=this._timeout),h.responseType=g?"arraybuffer":"text",d&&!this._nextRequestHeaders.has("Content-Type")&&("string"==typeof d?h["setRequestHeader"]("Content-Type","application/x-www-form-urlencoded"):h["setRequestHeader"]("Content-Type","application/octet-stream"));for(const[a,b]of this._nextRequestHeaders)try{h["setRequestHeader"](a,b)}catch(c){console.error(`[Construct 3] AJAX: Failed to set header '${a}: ${b}': `,c)}if(this._nextRequestHeaders.clear(),this._nextRequestOverrideMimeType){try{h["overrideMimeType"](this._nextRequestOverrideMimeType)}catch(a){console.error(`[Construct 3] AJAX: failed to override MIME type: `,a)}this._nextRequestOverrideMimeType=""}d?h.send(d):h.send()}catch(a){f(a)}})}GetDebuggerProperties(){return[{title:"plugins.ajax.debugger.title",properties:[{name:"plugins.ajax.debugger.last-data",value:this._lastData}]}]}SaveToJson(){return{"lastData":this._lastData}}LoadFromJson(a){this._lastData=a["lastData"],this._curTag="",this._progress=0}};

"use strict";C3.Plugins.AJAX.Cnds={OnComplete(a){return C3.equalsNoCase(this._curTag,a)},OnAnyComplete(){return!0},OnError(a){return C3.equalsNoCase(this._curTag,a)},OnAnyError(){return!0},OnProgress(a){return C3.equalsNoCase(this._curTag,a)}};

"use strict";C3.Plugins.AJAX.Acts={async Request(a,b){if(this._runtime.IsCordova()&&C3.IsRelativeURL(b))await this._DoCordovaRequest(a,b);else if(this._runtime.IsPreview()&&C3.IsRelativeURL(b)){const c=this._runtime.GetAssetManager().GetLocalUrlAsBlobUrl(b.toLowerCase());await this._DoRequest(a,c,"GET",null)}else await this._DoRequest(a,b,"GET",null)},async RequestFile(a,b){this._runtime.IsCordova()?await this._DoCordovaRequest(a,b):await this._DoRequest(a,this._runtime.GetAssetManager().GetLocalUrlAsBlobUrl(b),"GET",null)},async Post(a,b,c,d){await this._DoRequest(a,b,d,c)},async PostBinary(a,b,c,d){if(c){const e=c.GetFirstPicked(this._inst);if(e){const c=e.GetSdkInstance(),f=c.GetArrayBufferReadOnly();await this._DoRequest(a,b,d,f)}}},SetTimeout(a){this._timeout=1e3*a},SetHeader(a,b){this._nextRequestHeaders.set(a,b)},SetResponseBinary(a){if(a){const b=a.GetFirstPicked(this._inst);b&&(this._nextReponseBinaryData=b.GetSdkInstance())}},OverrideMIMEType(a){this._nextRequestOverrideMimeType=a}};
...
{
	C3.Plugins.Spriter = class Spriter extends C3.SDKPluginBase
	{
		constructor(opts)
		{
			super(opts);
...

{
	...
	C3.Plugins.Spriter.Instance = class SpriterInstance extends C3.SDKWorldInstanceBase
	{
		...
		//	this.runtime = type.runtime;
			...
			this.start_time = this._runtime.GetGameTime();
						...
						if (object.type == "reference")
						{
							...
							if (objState && typeof objState.frame !== 'undefined' && objState.frame > -1)
							{
								...
								if (obj)
								{
									...
									renderer.SetOpacity(this.opacity * objState.a);
		...
		onCreate()
		{
			...
			if ((!this.getCharacterFromPreload()) && this.properties[0].length > 0)
			{
				...
				// var url = await	LoadProjectFileUrl(this.properties[0]);
				...
				// {
					// this.runtime.fetchLocalFileViaCordovaAsText(this.runtime.getLocalFileUrl(this.properties[0]),
					// function (str)
					// {
				...
				// {
					...
					// {
						...
						// {
							...
							// {
								// log("[Construct 2] scml object(" + self.GetObjectClass().GetName(); + "): XMLHttpRequest failed with request status " + request.status + "when attempting to retrieve " + self.properties[0], "warn");
							// }
							// else
							// {
			...
			this.opacity = this.clamp(0.0, 1.0, this.properties[3] / 100.0);

			if (this.drawSelf)
			{
				// var frame = this.GetObjectClass().GetAnimations()[0].GetFrames()[0];
		...
		getNowTime()
		{
			return (this._runtime.GetGameTime() - this.start_time);
		};
		...
		tweenBone(bone, tweenedBones, animation, bones, currentIndex, currentTime, nextTime, key)
		{
			var nextBone = null;
			var nextFrame = null;
			var timelineIndex;
		...
		Tick2(ticklessRefresh)
		{
			...
			if (!this.animPlaying)
			{
				...
				{
					var currZ = this.findInArray(this, this.layer.instances);
					if (currZ !== this.lastZ)
					{
						changed = true;
						this.lastZ = currZ;
			...
			if (applyToInstances && !this.drawSelf)
			{
				...
				// {
					...
					// {
						...
						// {
							// var inst = c2Obj.inst;
							// if (inst && (this.setLayersForSprites || inst.GetWorldInfo()._SetLayer(this.GetWorldInfo().GetLayer())))
							// {
//								var currZ = this.findInArray(inst, instances);
								// var currZ = this.GetWorldInfo().GetZIndex();

								// if (currZ >= 1 && currZ <= zIndex)
								// {
									// this.GetWorldInfo()._SetZIndex(currZ);
				...
				tempZ = this.findInArray(this.GetInstance(), instances);
				...
				if (zIndex != tempZ)
				{
					...
					this.GetWorldInfo()._SetZIndex(zIndex);
					//instances.splice(zIndex, 0, this);
					//this.GetWorldInfo().GetLayer().SetZIndicesChanged();
					//this.GetWorldInfo().GetLayer()._SortInstancesByLastCachedZIndex();
			...
			for(var i=tempZ;i<instances.length;i++)
			{
				instances[i].GetWorldInfo()._SetZIndex(i+1000);
			}
			for (var i = 0; i < key.objects.length; i++)
			{
				object = key.objects[i];
				...
				if (c2Obj || refTimeline.objectType === "point")
				{
					...
					if ((inst || this.drawSelf) || refTimeline.objectType === "point" || refTimeline.objectType === "entity")
					{
						...
						{
							if(this.setLayersForSprites || inst.GetWorldInfo().GetLayer() == this.GetWorldInfo().GetLayer())
							{
								//var instZOrder = zIndex + 1 + zCounter++;
								//if (instances[instZOrder] !== inst)
								{
									inst.GetWorldInfo()._SetZIndex(zIndex+(i));
									//zCounter=zIndex+(i+1)+1;
			...
			if (c2ObjectArray)
			{
				for (var o = 0; o < c2ObjectArray.length; o++)
				{
					...
					else
					{
						...
						inst.GetWorldInfo().SetVisible(newState);
					}
				}
			}
		};
		...
		setAnimTo(animName, tick)
		{
			...
			var anim = this.currentAnimation;
			 if (anim && this.changeAnimTo != anim)
			 {
				 this.animPlaying = true;
			 }
		...
		associateTypeWithName(type, name)
		{
			...
			for (var o = 0, len = objectArray.length; o < len; o++)
			{
				...
				if (name == obj.name)
				{
					...
					var c2Object = c2ObjectArray[o];
					c2Object.type = type;
					var iid = this._inst.GetIID(); // get my IID
...
"use strict";{C3.Plugins.progressbar=class extends C3.SDKDOMPluginBase{constructor(a){super(a,"progress-bar"),this.AddElementMessageHandler("click",(a,b)=>a._OnClick(b))}Release(){super.Release()}}}

"use strict";C3.Plugins.progressbar.Type=class extends C3.SDKTypeBase{constructor(a){super(a)}Release(){super.Release()}OnCreate(){}};

"use strict";{const a=0;C3.Plugins.progressbar.Instance=class extends C3.SDKDOMInstanceBase{constructor(b,c){super(b,"progress-bar"),this._value=0,this._max=100,this._title="",this._id="",c&&(this._value=c[a],this._max=c[1],this._title=c[2],this.GetWorldInfo().SetVisible(c[3]),this._id=c[4]),this.CreateElement({"id":this._id})}Release(){super.Release()}GetElementState(){return{"value":this._value,"max":this._max,"title":this._title}}async _OnClick(){await this.TriggerAsync(C3.Plugins.progressbar.Cnds.OnClicked)}Draw(){}SaveToJson(){return{"v":this._value,"m":this._max,"t":this._title,"id":this._id}}LoadFromJson(a){this._value=a["v"],this._max=a["m"],this._title=a["t"],this._id=a["id"],this.UpdateElementState()}GetPropertyValueByIndex(a){return a===0?this._value:1===a?this._max:2===a?this._title:4===a?this._id:void 0}SetPropertyValueByIndex(b,c){switch(b){case a:if(this._value===c)return;this._value=c,this.UpdateElementState();break;case 1:if(this._max===c)return;this._max=c,this.UpdateElementState();break;case 2:if(this._title===c)return;this._title=c,this.UpdateElementState();break;case 4:if(this._id===c)return;this._id=c,this.UpdateElementState();}}GetDebuggerProperties(){const a=C3.Plugins.progressbar.Acts;return[{title:"plugins.progressbar.name",properties:[{name:"plugins.progressbar.properties.value.name",value:this._value,onedit:(b)=>this.CallAction(a.SetProgress,b)},{name:"plugins.progressbar.properties.maximum.name",value:this._max,onedit:(b)=>this.CallAction(a.SetMaximum,b)}]}]}}}

"use strict";C3.Plugins.progressbar.Cnds={OnClicked(){return!0},CompareProgress(a,b){return C3.compare(this._value,a,b)}};

"use strict";C3.Plugins.progressbar.Acts={SetTooltip(a){this._title===a||(this._title=a,this.UpdateElementState())},SetVisible(a){const b=this.GetWorldInfo();a=0!==a;b.IsVisible()===a||b.SetVisible(a)},SetCSSStyle(a,b){this.SetElementCSSStyle(a,b)},SetProgress(a){this._value===a||(this._value=a,this.UpdateElementState())},SetMaximum(a){this._max===a||(this._max=a,this.UpdateElementState())},SetIndeterminate(){this._max=0,this._value=0,this.UpdateElementState()}};

"use strict";C3.Plugins.progressbar.Exps={Progress(){return this._value},Maximum(){return this._max}};
...
"use strict";{function a(a,b,c){const d=a.GetSavedDataMap(),e=b.GetSavedDataMap();if(c){const c=d.get(g);c&&c.delete(b.GetSID());const f=e.get(g);f&&f.delete(a.GetSID())}else{let c=d.get(g);c||(c=new Set,d.set(g,c));let f=e.get(g);f||(f=new Set,e.set(g,f)),c.add(b.GetSID()),f.add(a.GetSID())}}function b(a){if(!(a instanceof IObjectClass))throw new TypeError("expected IObjectClass");const b=h.GetRuntime()._UnwrapScriptInterface(a);if(!b||!(b instanceof C3.ObjectClass))throw new Error("invalid IObjectClass");return b}let c=null,d=null,e=[],f=null,h=null;const g="Physics_DisabledCollisions";C3.Behaviors.Physics=class extends C3.SDKBehaviorBase{constructor(a){a.scriptInterfaceClass=IPhysicsBehavior,super(a),this._world=null,this._worldG=10,this._worldScale=.02,this._lastUpdateTick=-1,this._steppingMode=0,this._velocityIterations=8,this._positionIterations=3,this._allCollisionsEnabled=!0,this._runtime.AddLoadPromise(this._LoadBox2DWasm())}async _LoadBox2DWasm(){const a=await this._runtime.GetAssetManager().GetProjectFileUrl("box2d.wasm");await new Promise((b)=>{self["Box2DWasmModule"]({"wasmBinaryFile":a}).then((a)=>{f=a,this._InitBox2DWorld(),b()})})}_InitBox2DWorld(){const a=this._runtime.GetCollisionEngine();c=C3.Behaviors.Physics.GetVec2(0,0),d=C3.Behaviors.Physics.GetVec2(0,0),this._world=new f["b2World"](C3.Behaviors.Physics.GetTempVec2A(0,this._worldG),!0);const b=new f["JSContactListener"];b["BeginContact"]=(b)=>{const c=f["wrapPointer"](b,f["b2Contact"]),d=C3.Behaviors.Physics.Instance.LookupBehInstFromBody(c["GetFixtureA"]()["GetBody"]()),e=C3.Behaviors.Physics.Instance.LookupBehInstFromBody(c["GetFixtureB"]()["GetBody"]());a.RegisterCollision(d.GetObjectInstance(),e.GetObjectInstance())},b["EndContact"]=()=>{},b["PreSolve"]=()=>{},b["PostSolve"]=()=>{},this._world["SetContactListener"](b);const e=new f["JSContactFilter"];e["ShouldCollide"]=(a,b)=>{if(this._allCollisionsEnabled)return!0;const c=f["wrapPointer"](a,f["b2Fixture"]),d=f["wrapPointer"](b,f["b2Fixture"]),e=C3.Behaviors.Physics.Instance.LookupBehInstFromBody(c["GetBody"]()),h=C3.Behaviors.Physics.Instance.LookupBehInstFromBody(d["GetBody"]()),i=e.GetObjectInstance().GetObjectClass(),j=h.GetObjectInstance().GetObjectClass(),k=i.GetSID(),l=j.GetSID(),m=i.GetSavedDataMap().get(g);if(m&&m.has(l))return!1;const n=j.GetSavedDataMap().get(g);return!(n&&n.has(k))},this._world["SetContactFilter"](e)}Release(){super.Release()}GetBox2D(){return f}GetWorld(){return this._world}GetWorldScale(){return this._worldScale}GetSteppingMode(){return this._steppingMode}SetSteppingMode(a){this._steppingMode=a}SetLastUpdateTick(a){this._lastUpdateTick=a}GetLastUpdateTick(){return this._lastUpdateTick}SetVelocityIterations(a){this._velocityIterations=Math.max(a,1)}GetVelocityIterations(){return this._velocityIterations}SetPositionIterations(a){this._positionIterations=Math.max(a,1)}GetPositionIterations(){return this._positionIterations}SetIterations(a,b){this.SetVelocityIterations(a),this.SetPositionIterations(b)}GetGravity(){return this._worldG}SetGravity(a){a===this._worldG||(this._world["SetGravity"](C3.Behaviors.Physics.GetTempVec2A(0,a)),this._worldG=a,this._WakeUpAllPhysicsBodies())}_WakeUpAllPhysicsBodies(){for(const a of this.GetInstances()){const b=C3.Behaviors.Physics.Instance.LookupBehInstFromInst(a);if(!b)continue;const c=b.GetBody();c&&c["SetAwake"](!0)}}DisableShouldCollideFastPath(){this._allCollisionsEnabled=!1}SetCollisionsEnabled(b,c,d){if(d=!!d,b&&c){if(c.IsFamily())for(const e of c.GetFamilyMembers())a(b,e,d);else a(b,c,d);this.DisableShouldCollideFastPath()}}static GetPhysicsCollisionKey(){return g}static GetVec2(a,b){if(e.length){const c=e.pop();return c["set_x"](a),c["set_y"](b),c}else{const c=f["b2Vec2"];return new c(a,b)}}static FreeVec2(a){e.push(a)}static GetTempVec2A(a,b){return c["set_x"](a),c["set_y"](b),c}static GetTempVec2B(a,b){return d["set_x"](a),d["set_y"](b),d}static CreatePolygonShape(a){const b=f["b2PolygonShape"],c=new b,d=f["_malloc"](8*a.length);let e=0;for(let b=0;b<a.length;++b)f["HEAPF32"][d+e>>2]=a[b]["get_x"](),f["HEAPF32"][d+(e+4)>>2]=a[b]["get_y"](),e+=8;const g=f["wrapPointer"](d,f["b2Vec2"]);return c["Set"](g,a.length),f["_free"](d),c}};const j=["fixed","variable"];self.IPhysicsBehavior=class extends IBehavior{constructor(a){super(a),h=a}set worldGravity(a){h.SetGravity(a)}get worldGravity(){return h.GetGravity()}set steppingMode(a){const b=j.indexOf(a);if(0>b)throw new Error("invalid stepping mode");h.SetSteppingMode(j[b])}get steppingMode(){return j[h.GetSteppingMode()]}set velocityIterations(a){h.SetVelocityIterations(a)}get velocityIterations(){return h.GetVelocityIterations()}set positionIterations(a){h.SetPositionIterations(a)}get positionIterations(){return h.GetPositionIterations()}setCollisionsEnabled(a,c,d){const e=b(a),f=b(c);d=!!d,h.SetCollisionsEnabled(e,f,d)}}}
...
"use strict";{function a(a,b){if(!(b instanceof IWorldInstance))throw new TypeError("expected IWorldInstance");const c=g.get(a),d=c.GetRuntime()._UnwrapScriptInterface(b);if(!d||!(d instanceof C3.Instance))throw new Error("invalid IInstance");return d}const b=new WeakMap,c=new WeakMap,d=new WeakMap,e=C3.Behaviors.Physics.GetTempVec2A,f=C3.Behaviors.Physics.GetTempVec2B;C3.Behaviors.Physics.Instance=class extends C3.SDKBehaviorInstanceBase{constructor(a,b){super(a);const d=this.GetBehavior(),e=this.GetWorldInfo();this._box2d=d.GetBox2D(),this._world=d.GetWorld(),this._worldScale=d.GetWorldScale(),this._isImmovable=!1,this._collisionMask=0,this._preventRotation=!1,this._density=1,this._friction=.5,this._restitution=.2,this._linearDamping=0,this._angularDamping=.01,this._isBullet=!1,this._isEnabled=!0,this._body=null,this._fixtures=[],this._myJoints=[],this._myCreatedJoints=[],this._joiningMe=new Set,this._lastKnownX=e.GetX(),this._lastKnownY=e.GetY(),this._lastKnownAngle=e.GetAngle(),this._lastWidth=0,this._lastHeight=0,this._lastTickOverride=!1,this._recreateBody=!1,this._lastImageInfo=null,b&&(this._isImmovable=!!b[0],this._collisionMask=b[1],this._preventRotation=!!b[2],this._density=b[3],this._friction=b[4],this._restitution=b[5],this._linearDamping=b[6],this._angularDamping=b[7],this._isBullet=!!b[8],this._isEnabled=!!b[9]);const f=this._runtime.Dispatcher();this._disposables=new C3.CompositeDisposable(C3.Disposable.From(f,"instancedestroy",(a)=>this._OnInstanceDestroyed(a.instance)),C3.Disposable.From(f,"afterload",()=>this._OnAfterLoad())),c.set(this._inst,this),this._isEnabled&&this._StartTicking()}PostCreate(){this._CreateBody(),this._lastImageInfo=this._inst.GetCurrentImageInfo()}Release(){this._DestroyMyJoints(),C3.clearArray(this._myCreatedJoints),this._joiningMe.clear(),this._body&&(this._DestroyFixtures(),this._world["DestroyBody"](this._body),this._body=null),super.Release()}_CreateFixture(a){if(this._body){const b=this._body["CreateFixture"](a);return this._fixtures.push(b),b}}_DestroyFixtures(){if(this._body){for(const a of this._fixtures)this._body["DestroyFixture"](a);C3.clearArray(this._fixtures)}}_Destroy(a){this._box2d["destroy"](a)}_CreateBody(){var a=Math.max,c=Math.abs;if(!this._isEnabled)return;const d=this._box2d["b2FixtureDef"],e=this._box2d["b2BodyDef"],g=this.GetWorldInfo();if(!this._body){const a=new e;a["set_type"](this._isImmovable?0:2);const c=g.GetBoundingQuad();a["set_position"](f(c.midX()*this._worldScale,c.midY()*this._worldScale)),a["set_angle"](g.GetAngle()),a["set_fixedRotation"](this._preventRotation),a["set_linearDamping"](this._linearDamping),a["set_angularDamping"](this._angularDamping),a["set_bullet"](this._isBullet),this._body=this._world["CreateBody"](a),this._Destroy(a),b.set(this._body,this)}this._DestroyFixtures();const h=new d;h["set_density"](this._density),h["set_friction"](this._friction),h["set_restitution"](this._restitution);const j=g.HasOwnCollisionPoly();let k=this._collisionMask;j||this._inst.HasTilemap()||0!==k||(k=1);const l=a(c(g.GetWidth()),1),m=a(c(g.GetHeight()),1);0===k?this._inst.HasTilemap()?this._CreateTilemapFixtures(h):this._CreatePolygonFixture(h,l,m):1===k?this._CreateBoundingBoxFixture(h,l,m):this._CreateCircleFixture(h,l,m),this._lastWidth=g.GetWidth(),this._lastHeight=g.GetHeight(),this._Destroy(h)}_CreateBoundingBoxFixture(a,b,c){const d=this._box2d["b2PolygonShape"],e=new d;e["SetAsBox"](.5*(b*this._worldScale),.5*(c*this._worldScale)),a["set_shape"](e),this._CreateFixture(a),this._Destroy(e)}_CreateCircleFixture(a,b,c){const d=this._box2d["b2CircleShape"],e=new d;e["set_m_radius"](.5*(Math.min(b,c)*this._worldScale)),a["set_shape"](e),this._CreateFixture(a),this._Destroy(e)}_CreatePolygonFixture(a,b,c){const d=this.GetWorldInfo(),e=0>d.GetWidth(),f=0>d.GetHeight(),g=this._worldScale,h=d.GetAngle();0!==h&&(d.SetAngle(0),d.SetBboxChanged());const j=d.GetBoundingQuad(),k=j.midX()-d.GetX(),l=j.midY()-d.GetY();0!==h&&(d.SetAngle(h),d.SetBboxChanged());const m=d.GetCustomTransformedCollisionPoly(e?-b:b,f?-c:c,0),n=m.pointsArr(),o=n.length/2,p=[];for(let d=0;d<o;++d)p.push(C3.Behaviors.Physics.GetVec2(n[2*d]-k,n[2*d+1]-l));e!=f&&p.reverse();const q=C3.Behaviors.Physics.Separator.Separate(p,b*c);for(const d of p)C3.Behaviors.Physics.FreeVec2(d);if(q.length)for(const b of q){for(const a of b)a["set_x"](a["get_x"]()*g),a["set_y"](a["get_y"]()*g);const c=C3.Behaviors.Physics.CreatePolygonShape(b);a["set_shape"](c),this._CreateFixture(a),this._Destroy(c);for(const a of b)C3.Behaviors.Physics.FreeVec2(a)}else this._CreateBoundingBoxFixture(a,b,c)}_CreateTilemapFixtures(a){const b=this.GetWorldInfo(),c=b.GetBoundingQuad(),e=c.midX()-b.GetX(),f=c.midY()-b.GetY(),g=this._worldScale,h=C3.Behaviors.Physics.GetVec2,j=C3.Behaviors.Physics.FreeVec2,k=[];this._inst.GetSdkInstance().GetAllCollisionRects(k);const l=[];for(let b=0,c=k.length;b<c;++b){const m=k[b],c=m.GetRect(),n=m.GetPoly();if(n){let b=d.get(n);if(!b){const a=n.pointsArr(),e=n.pointCount();for(let b=0;b<e;++b)l.push(h(a[2*b],a[2*b+1]));const f=m.GetTileId()&3758096384;(f==-2147483648||f==1073741824||f==536870912||f&-2147483648&&f&1073741824&&f&536870912)&&l.reverse(),b=C3.Behaviors.Physics.Separator.Separate(l,c.width()*c.height()),d.set(n,b);for(const a of l)j(a);C3.clearArray(l)}for(let d=0,k=b.length;d<k;++d){const m=b[d];for(let a=0,b=m.length;a<b;++a)l.push(h((c.getLeft()+m[a]["get_x"]()-e)*g,(c.getTop()+m[a]["get_y"]()-f)*g));const k=C3.Behaviors.Physics.CreatePolygonShape(l);a["set_shape"](k),this._CreateFixture(a),this._Destroy(k);for(const a of l)j(a);C3.clearArray(l)}}else{l.push(h((c.getLeft()-e)*g,(c.getTop()-f)*g)),l.push(h((c.getRight()-e)*g,(c.getTop()-f)*g)),l.push(h((c.getRight()-e)*g,(c.getBottom()-f)*g)),l.push(h((c.getLeft()-e)*g,(c.getBottom()-f)*g));const b=C3.Behaviors.Physics.CreatePolygonShape(l);a["set_shape"](b),this._CreateFixture(a),this._Destroy(b)}for(const a of l)j(a);C3.clearArray(l)}}_DestroyBody(){this._body&&(this._DestroyMyJoints(),b.delete(this._body),this._DestroyFixtures(),this._world["DestroyBody"](this._body),this._body=null)}_DestroyMyJoints(){for(const a of this._myJoints)this._world["DestroyJoint"](a);C3.clearArray(this._myJoints)}_RecreateMyJoints(){for(const a of this._myCreatedJoints)switch(a.type){case 0:this._DoCreateDistanceJoint(...a.params);break;case 1:this._DoCreateRevoluteJoint(...a.params);break;case 2:this._DoCreateLimitedRevoluteJoint(...a.params);break;case 3:this._DoCreatePrismaticJoint(...a.params);break;default:;}}_GetInstImagePoint(a){const b=this.GetWorldInfo();if(-1===a)return[b.GetX(),b.GetY()];if(0===a&&this._body){const a=this._body["GetPosition"](),b=this._body["GetLocalCenter"]();return[(a["get_x"]()+b["get_x"]())/this._worldScale,(a["get_y"]()+b["get_y"]())/this._worldScale]}return this._inst.GetImagePoint(a)}_CreateDistanceJoint(a,b,c,d,e){this._isEnabled&&b&&b!==this._inst&&C3.Behaviors.Physics.Instance.LookupBehInstFromInst(b)&&(this._myCreatedJoints.push({type:0,params:[a,b.GetUID(),c,d,e]}),this._DoCreateDistanceJoint(a,b.GetUID(),c,d,e))}_DoCreateDistanceJoint(a,b,d,g,h){if(this._isEnabled){const j=this._runtime.GetInstanceByUID(b);if(j&&j!==this._inst&&c.has(j)){const b=C3.Behaviors.Physics.Instance.LookupBehInstFromInst(j);b._joiningMe.add(this._inst),this._UpdateBodyToMatchInstance(!1),b._UpdateBodyToMatchInstance(!1);const[c,k]=this._GetInstImagePoint(a),[l,m]=j.GetImagePoint(d),n=this._box2d["b2DistanceJointDef"],o=this._worldScale,p=new n;p["Initialize"](this._body,b.GetBody(),e(c*o,k*o),f(l*o,m*o)),p["set_length"](Math.hypot(c-l,k-m)*o),p["set_dampingRatio"](g),p["set_frequencyHz"](h),this._myJoints.push(this._world["CreateJoint"](p)),this._Destroy(p)}}}_CreateRevoluteJoint(a,b){this._isEnabled&&b&&b!==this._inst&&C3.Behaviors.Physics.Instance.LookupBehInstFromInst(b)&&(this._myCreatedJoints.push({type:1,params:[a,b.GetUID()]}),this._DoCreateRevoluteJoint(a,b.GetUID()))}_DoCreateRevoluteJoint(a,b){if(this._isEnabled){const d=this._runtime.GetInstanceByUID(b);if(d&&d!==this._inst&&c.has(d)){const b=C3.Behaviors.Physics.Instance.LookupBehInstFromInst(d);b._joiningMe.add(this._inst),this._UpdateBodyToMatchInstance(!1),b._UpdateBodyToMatchInstance(!1);const[c,f]=this._GetInstImagePoint(a),g=this._box2d["b2RevoluteJointDef"],h=this._worldScale,j=new g;j["Initialize"](this._body,b.GetBody(),e(c*h,f*h)),this._myJoints.push(this._world["CreateJoint"](j)),this._Destroy(j)}}}_CreateLimitedRevoluteJoint(a,b,c,d){this._isEnabled&&b&&b!==this._inst&&C3.Behaviors.Physics.Instance.LookupBehInstFromInst(b)&&(c=C3.toDegrees(c),d=C3.toDegrees(d),this._myCreatedJoints.push({type:2,params:[a,b.GetUID(),c,d]}),this._DoCreateLimitedRevoluteJoint(a,b.GetUID(),c,d))}_DoCreateLimitedRevoluteJoint(a,b,d,f){if(this._isEnabled){const g=this._runtime.GetInstanceByUID(b);if(g&&g!==this._inst&&c.has(g)){const b=C3.Behaviors.Physics.Instance.LookupBehInstFromInst(g);b._joiningMe.add(this._inst),this._UpdateBodyToMatchInstance(!1),b._UpdateBodyToMatchInstance(!1);const[c,h]=this._GetInstImagePoint(a),j=this._box2d["b2RevoluteJointDef"],k=this._worldScale,l=new j;l["Initialize"](this._body,b.GetBody(),e(c*k,h*k)),l["set_enableLimit"](!0),l["set_lowerAngle"](C3.toRadians(d)),l["set_upperAngle"](C3.toRadians(f)),this._myJoints.push(this._world["CreateJoint"](l)),this._Destroy(l)}}}_CreatePrismaticJoint(a,b,c,d,e,f,g,h,j){this._isEnabled&&b&&b!==this._inst&&C3.Behaviors.Physics.Instance.LookupBehInstFromInst(b)&&(c=C3.toDegrees(c),h=C3.toDegrees(h),this._myCreatedJoints.push({type:3,params:[a,b.GetUID(),c,d,e,f,g,h,j]}),this._DoCreatePrismaticJoint(a,b.GetUID(),c,d,e,f,g,h,j))}_DoCreatePrismaticJoint(a,b,d,g,h,j,k,l,m){if(this._isEnabled){const n=this._runtime.GetInstanceByUID(b);if(n&&n!==this._inst&&c.has(n)){const b=C3.Behaviors.Physics.Instance.LookupBehInstFromInst(n);b._joiningMe.add(this._inst),this._UpdateBodyToMatchInstance(!1),b._UpdateBodyToMatchInstance(!1);const[c,o]=this._GetInstImagePoint(a);d=C3.toRadians(d);const p=Math.cos(d),q=Math.sin(d),r=this._box2d["b2PrismaticJointDef"],s=this._worldScale,t=new r;t["Initialize"](this._body,b.GetBody(),e(c*s,o*s),f(p,q)),t["set_enableLimit"](!!g),t["set_lowerTranslation"](h*s),t["set_upperTranslation"](j*s),t["set_enableMotor"](!!k),t["set_motorSpeed"](C3.toRadians(l)),t["set_maxMotorForce"](m),this._myJoints.push(this._world["CreateJoint"](t)),this._Destroy(t)}}}_RemoveJoints(){this._isEnabled&&(this._DestroyMyJoints(),C3.clearArray(this._myCreatedJoints),this._joiningMe.clear())}_OnInstanceDestroyed(a){const b=a.GetUID();let c=0;for(let d=0,e=this._myCreatedJoints.length;d<e;++d)this._myCreatedJoints[c]=this._myCreatedJoints[d],c<this._myJoints.length&&(this._myJoints[c]=this._myJoints[d]),this._myCreatedJoints[d].params[1]===b?d<this._myJoints.length&&this._world["DestroyJoint"](this._myJoints[d]):++c;C3.truncateArray(this._myCreatedJoints,c),c<this._myJoints.length&&C3.truncateArray(this._myJoints,c),this._joiningMe.delete(a)}GetBody(){return this._body}static LookupBehInstFromBody(a){return b.get(a)||null}static LookupBehInstFromInst(a){return c.get(a)||null}SaveToJson(){const a={"e":this._isEnabled,"pr":this._preventRotation,"d":this._density,"fr":this._friction,"re":this._restitution,"ld":this._linearDamping,"ad":this._angularDamping,"b":this._isBullet,"mcj":this._myCreatedJoints};if(this._isEnabled){const b=this._body["GetLinearVelocity"]();a["vx"]=b["get_x"](),a["vy"]=b["get_y"](),a["om"]=this._body["GetAngularVelocity"]()}return a}LoadFromJson(a){this._DestroyMyJoints(),C3.clearArray(this._myCreatedJoints),this._joiningMe.clear(),this._DestroyBody(),this._isEnabled=a["e"],this._preventRotation=a["pr"],this._density=a["d"],this._friction=a["fr"],this._restitution=a["re"],this._linearDamping=a["ld"],this._angularDamping=a["ad"],this._isBullet=a["b"],this._myCreatedJoints=a["mcj"];const b=this.GetWorldInfo();this._lastKnownX=b.GetX(),this._lastKnownY=b.GetY(),this._lastKnownAngle=b.GetAngle(),this._lastWidth=b.GetWidth(),this._lastHeight=b.GetHeight(),this._isEnabled&&(this._CreateBody(),this._body["SetLinearVelocity"](e(a["vx"],a["vy"])),this._body["SetAngularVelocity"](a["om"]),(0!==a["vx"]||0!==a["vy"]||0!==a["om"])&&this._body["SetAwake"](!0),this._myCreatedJoints=a["mcj"]),this._isEnabled?this._StartTicking():this._StopTicking()}_OnAfterLoad(){this._isEnabled&&this._RecreateMyJoints()}Tick(){if(!this._isEnabled)return;const a=this._runtime,b=this.GetBehavior();let c=0;0===b.GetSteppingMode()?c=a.GetTimeScale()/60:(c=a.GetDt(this._inst),c>1/30&&(c=1/30));const d=a.GetTickCountNoSave();if(d>b.GetLastUpdateTick()&&0<a.GetTimeScale()){const a=this._runtime.IsDebug();let e=0;a&&(e=performance.now()),0!==c&&this._world["Step"](c,b.GetVelocityIterations(),b.GetPositionIterations()),this._world["ClearForces"](),a&&C3Debugger.AddPhysicsTime(performance.now()-e),b.SetLastUpdateTick(d)}this._UpdateBodyToMatchInstance(!0)}_UpdateBodyToMatchInstance(a){const b=this._inst,c=b.GetWorldInfo(),d=this._worldScale;(this._recreateBody||c.GetWidth()!==this._lastWidth||c.GetHeight()!==this._lastHeight||b.GetCurrentImageInfo()!==this._lastImageInfo||b.HasTilemap()&&b.GetUnsavedDataMap().get("PhysicsChanged"))&&(this._CreateBody(),this._recreateBody=!1,this._lastImageInfo=b.GetCurrentImageInfo(),b.HasTilemap()&&b.GetUnsavedDataMap().set("PhysicsChanged",!1));const f=this._body,g=c.GetX()!==this._lastKnownX||c.GetY()!==this._lastKnownY,h=c.GetAngle()!==this._lastKnownAngle;if(g){const b=c.GetBoundingQuad(),g=b.midX(),j=b.midY(),k=g-this._lastKnownX,l=j-this._lastKnownY;h?f["SetTransform"](e(g*d,j*d),c.GetAngle()):f["SetTransform"](e(g*d,j*d),f["GetAngle"]()),a&&(f["SetLinearVelocity"](e(k,l)),this._lastTickOverride=!0),f["SetAwake"](!0)}else if(a&&this._lastTickOverride){const a=c.GetBoundingQuad();this._lastTickOverride=!1,f["SetLinearVelocity"](e(0,0)),f["SetTransform"](e(a.midX()*d,a.midY()*d),f["GetAngle"]())}!g&&h&&(f["SetTransform"](f["GetPosition"](),c.GetAngle()),f["SetAwake"](!0));const j=f["GetPosition"](),k=j["get_x"]()/d,l=j["get_y"]()/d,m=f["GetAngle"]();if(k!==c.GetX()||l!==c.GetY()||m!==c.GetAngle()){c.SetXY(k,l),c.SetAngle(m),c.SetBboxChanged();const a=c.GetBoundingQuad(),b=a.midX()-c.GetX(),d=a.midY()-c.GetY();(0!=b||0!=d)&&(c.OffsetXY(-b,-d),c.SetBboxChanged())}this._lastKnownX=c.GetX(),this._lastKnownY=c.GetY(),this._lastKnownAngle=c.GetAngle()}GetPropertyValueByIndex(a){return 2===a?this._IsPreventRotate():3===a?this._GetDensity():4===a?this._GetFriction():5===a?this._GetElasticity():6===a?this._GetLinearDamping():7===a?this._GetAngularDamping():8===a?this._IsBullet():9===a?this._IsEnabled():void 0}SetPropertyValueByIndex(a,b){2===a?this._SetPreventRotate(b):3===a?this._SetDensity(b):4===a?this._SetFriction(b):5===a?this._SetElasticity(b):6===a?this._SetLinearDamping(b):7===a?this._SetAngularDamping(b):8===a?this._SetBullet(b):9===a?this._SetEnabled(b):void 0}_SetEnabled(a){a=!!a,this._isEnabled&&!a?(this._DestroyBody(),this._isEnabled=!1,this._StopTicking()):!this._isEnabled&&a&&(this._isEnabled=!0,this._CreateBody(),this._StartTicking())}_IsEnabled(){return this._isEnabled}GetDebuggerProperties(){const a=[{name:"behaviors.physics.properties.enabled.name",value:this._IsEnabled(),onedit:(a)=>this._SetEnabled(a)},{name:"behaviors.physics.properties.immovable.name",value:this._IsImmovable(),onedit:(a)=>this._SetImmovable(a)},{name:"behaviors.physics.properties.density.name",value:this._GetDensity(),onedit:(a)=>this._SetDensity(a)},{name:"behaviors.physics.properties.friction.name",value:this._GetFriction(),onedit:(a)=>this._SetFriction(a)},{name:"behaviors.physics.properties.elasticity.name",value:this._GetElasticity(),onedit:(a)=>this._SetElasticity(a)},{name:"behaviors.physics.properties.linear-damping.name",value:this._GetLinearDamping(),onedit:(a)=>this._SetLinearDamping(a)},{name:"behaviors.physics.properties.angular-damping.name",value:this._GetAngularDamping(),onedit:(a)=>this._SetAngularDamping(a)}];return this._isEnabled&&(a.push({name:"behaviors.physics.debugger.is-sleeping",value:this._IsSleeping()}),a.push({name:"behaviors.physics.debugger.velocity-x",value:this._GetVelocityX(),onedit:(a)=>this._SetVelocity(a,this._GetVelocityY())}),a.push({name:"behaviors.physics.debugger.velocity-y",value:this._GetVelocityY(),onedit:(a)=>this._SetVelocity(this._GetVelocityX(),a)}),a.push({name:"behaviors.physics.debugger.angular-velocity",value:C3.toDegrees(this._GetAngularVelocity()),onedit:(a)=>this._SetAngularVelocity(C3.toRadians(a))}),a.push({name:"behaviors.physics.debugger.mass",value:this._GetMass()})),[{title:"$"+this.GetBehaviorType().GetName(),properties:a}]}_ApplyForce(a,b,c){const[d,e]=this._GetInstImagePoint(c);this._DoApplyForce(a,b,d,e)}_ApplyForceToward(b,c,d,e){const[f,g]=this._GetInstImagePoint(e),h=C3.angleTo(f,g,c,d);this._DoApplyForce(Math.cos(h)*b,Math.sin(h)*b,f,g)}_ApplyForceAtAngle(b,c,a){const[d,e]=this._GetInstImagePoint(a);this._DoApplyForce(Math.cos(c)*b,Math.sin(c)*b,d,e)}_DoApplyForce(a,b,c,d){if(this._isEnabled){const g=this._worldScale;this._body["ApplyForce"](e(a,b),f(c*g,d*g),!0)}}_ApplyImpulse(a,b,c){const[d,e]=this._GetInstImagePoint(c);this._DoApplyImpulse(a,b,d,e)}_ApplyImpulseToward(b,c,d,e){const[f,g]=this._GetInstImagePoint(e),h=C3.angleTo(f,g,c,d);this._DoApplyImpulse(Math.cos(h)*b,Math.sin(h)*b,f,g)}_ApplyImpulseAtAngle(b,c,a){const[d,e]=this._GetInstImagePoint(a);this._DoApplyImpulse(Math.cos(c)*b,Math.sin(c)*b,d,e)}_DoApplyImpulse(a,b,c,d){if(this._isEnabled){const g=this._worldScale;this._body["ApplyLinearImpulse"](e(a,b),f(c*g,d*g),!0);const h=this.GetWorldInfo();this._lastKnownX=h.GetX(),this._lastKnownY=h.GetY(),this._lastTickOverride=!1}}_ApplyTorque(a){this._isEnabled&&this._body["ApplyTorque"](a,!0)}_ApplyTorqueToAngle(b,c){const a=C3.angleClockwise(this.GetWorldInfo().GetAngle(),c)?-1:1;this._ApplyTorque(b*a)}_ApplyTorqueToPosition(b,c,d){const e=this.GetWorldInfo(),g=C3.angleTo(e.GetX(),e.GetY(),c,d),a=C3.angleClockwise(e.GetAngle(),g)?-1:1;this._ApplyForce(b*a)}_SetAngularVelocity(a){this._isEnabled&&(this._body["SetAngularVelocity"](a),this._body["SetAwake"](!0))}_GetAngularVelocity(){return this._isEnabled?this._body["GetAngularVelocity"]():0}_SetVelocity(a,b){if(this._isEnabled){const c=this._worldScale;this._body["SetLinearVelocity"](e(a*c,b*c)),this._body["SetAwake"](!0);const d=this.GetWorldInfo();this._lastKnownX=d.GetX(),this._lastKnownY=d.GetY(),this._lastTickOverride=!1}}_GetVelocity(){if(!this._isEnabled)return[0,0];const a=this._worldScale,b=this._body["GetLinearVelocity"]();return[b["get_x"]()/a,b["get_y"]()/a]}_GetVelocityX(){return this._isEnabled?this._body["GetLinearVelocity"]()["get_x"]()/this._worldScale:0}_GetVelocityY(){return this._isEnabled?this._body["GetLinearVelocity"]()["get_y"]()/this._worldScale:0}_SetDensity(a){if(this._isEnabled&&this._density!==a){this._density=a;for(const b of this._fixtures)b["SetDensity"](a);this._body["ResetMassData"]()}}_GetDensity(){return this._isEnabled?this._density:0}_SetFriction(a){if(this._isEnabled&&this._friction!==a){this._friction=a;for(const b of this._fixtures)b["SetFriction"](a);for(let a=this._body["GetContactList"]();this._box2d["getPointer"](a);a=a["get_next"]()){const b=a["get_contact"]();b&&b["ResetFriction"]()}}}_GetFriction(){return this._isEnabled?this._friction:0}_SetElasticity(a){if(this._isEnabled&&this._restitution!==a){this._restitution=a;for(const b of this._fixtures)b["SetRestitution"](a)}}_GetElasticity(){return this._isEnabled?this._restitution:0}_SetLinearDamping(a){this._isEnabled&&this._linearDamping!==a&&(this._linearDamping=a,this._body["SetLinearDamping"](a))}_GetLinearDamping(){return this._isEnabled?this._linearDamping:0}_SetAngularDamping(a){this._isEnabled&&this._angularDamping!==a&&(this._angularDamping=a,this._body["SetAngularDamping"](a))}_GetAngularDamping(){return this._isEnabled?this._angularDamping:0}_SetImmovable(a){this._isEnabled&&(a=!!a,this._isImmovable===a||(this._isImmovable=a,this._body["SetType"](this._isImmovable?0:2),this._body["SetAwake"](!0)))}_IsImmovable(){return this._isImmovable}_SetPreventRotate(a){this._isEnabled&&(a=!!a,this._preventRotation===a||(this._preventRotation=a,this._body["SetFixedRotation"](this._preventRotation),this._body["SetAngularVelocity"](0),this._body["SetAwake"](!0)))}_IsPreventRotate(){return this._preventRotation}_SetBullet(a){this._isEnabled&&(a=!!a,this._isBullet===a||(this._isBullet=a,this._body["SetBullet"](this._isBullet),this._body["SetAwake"](!0)))}_IsBullet(){return this._isBullet}_GetMass(){return this._isEnabled?this._body["GetMass"]()/this._worldScale:0}_GetCenterOfMassX(){return this._isEnabled?(this._body["GetPosition"]()["get_x"]()+this._body["GetLocalCenter"]()["get_x"]())/this._worldScale:0}_GetCenterOfMassY(){return this._isEnabled?(this._body["GetPosition"]()["get_y"]()+this._body["GetLocalCenter"]()["get_y"]())/this._worldScale:0}_GetCenterOfMass(){if(!this._isEnabled)return[0,0];const a=this._body["GetPosition"](),b=this._body["GetLocalCenter"](),c=this._worldScale;return[(a["get_x"]()+b["get_x"]())/c,(a["get_y"]()+b["get_y"]())/c]}_IsSleeping(){return!!this._isEnabled&&!this._body["IsAwake"]()}GetScriptInterfaceClass(){return IPhysicsBehaviorInstance}};const g=new WeakMap;self.IPhysicsBehaviorInstance=class extends IBehaviorInstance{constructor(){super(),g.set(this,IBehaviorInstance._GetInitInst().GetSdkInstance())}get isEnabled(){return g.get(this)._IsEnabled()}set isEnabled(a){g.get(this)._SetEnabled(a)}applyForce(a,b,c=0){g.get(this)._ApplyForce(a,b,c)}applyForceTowardPosition(a,b,c,d=0){g.get(this)._ApplyForceToward(a,b,c,d)}applyForceAtAngle(b,c,a=0){g.get(this)._ApplyForceAtAngle(b,c,a)}applyImpulse(a,b,c=0){g.get(this)._ApplyImpulse(a,b,c)}applyImpulseTowardPosition(a,b,c,d=0){g.get(this)._ApplyImpulseToward(a,b,c,d)}applyImpulseAtAngle(b,c,a=0){g.get(this)._ApplyImpulseAtAngle(b,c,a)}applyTorque(a){g.get(this)._ApplyTorque(a)}applyTorqueToAngle(b,c){g.get(this)._ApplyTorqueToAngle(b,c)}applyTorqueToPosition(a,b,c){g.get(this)._ApplyTorqueToPosition(a,b,c)}set angularVelocity(a){g.get(this)._SetAngularVelocity(a)}get angularVelocity(){return g.get(this)._GetAngularVelocity()}setVelocity(a,b){g.get(this)._SetVelocity(a,b)}getVelocityX(){return g.get(this)._GetVelocityX()}getVelocityY(){return g.get(this)._GetVelocityY()}getVelocity(){return g.get(this)._GetVelocity()}set density(a){g.get(this)._SetDensity(a)}get density(){return g.get(this)._GetDensity()}set friction(a){g.get(this)._SetFriction(a)}get friction(){return g.get(this)._GetFriction()}set elasticity(a){g.get(this)._SetElasticity(a)}get elasticity(){return g.get(this)._GetElasticity()}set linearDamping(a){g.get(this)._SetLinearDamping(a)}get linearDamping(){return g.get(this)._GetLinearDamping()}set angularDamping(a){g.get(this)._SetAngularDamping(a)}get angularDamping(){return g.get(this)._GetAngularDamping()}set isImmovable(a){g.get(this)._SetImmovable(a)}get isImmovable(){return g.get(this)._IsImmovable()}set isPreventRotation(a){g.get(this)._SetPreventRotate(a)}get isPreventRotation(){return g.get(this)._IsPreventRotate()}set isBullet(a){g.get(this)._SetBullet(i)}get isBullet(){return g.get(this)._IsBullet()}get mass(){return g.get(this)._GetMass()}getCenterOfMassX(){return g.get(this)._GetCenterOfMassX()}getCenterOfMassY(){return g.get(this)._GetCenterOfMassY()}getCenterOfMass(){return g.get(this)._GetCenterOfMass()}get isSleeping(){return g.get(this)._IsSleeping()}createDistanceJoint(b,c,d,e,f){const h=a(this,c);g.get(this)._CreateDistanceJoint(b,h,d,e,f)}createRevoluteJoint(b,c){const d=a(this,c);g.get(this)._CreateRevoluteJoint(b,d)}createLimitedRevoluteJoint(b,c,d,e){const f=a(this,c);g.get(this)._CreateLimitedRevoluteJoint(b,f,d,e)}createPrismaticJoint(b,c,d,e,f,h,j,k,l){const m=a(this,c);g.get(this)._CreatePrismaticJoint(b,m,d,e,f,h,j,k,l)}removeAllJoints(){g.get(this)._RemoveJoints()}}}

"use strict";C3.Behaviors.Physics.Cnds={IsSleeping(){return this._IsSleeping()},CompareVelocity(a,b,c){if(!this._isEnabled)return!1;let d=0;if(0===a)d=this._GetVelocityX();else if(1===a)d=this._GetVelocityY();else{const[a,b]=this._GetVelocity();d=Math.hypot(a,b)}return C3.compare(d,b,c)},CompareAngularVelocity(a,b){if(!this._isEnabled)return!1;const c=C3.toDegrees(this._GetAngularVelocity());return C3.compare(c,a,b)},CompareMass(a,b){if(!this._isEnabled)return!1;const c=this._GetMass();return C3.compare(c,a,b)},IsEnabled(){return this._IsEnabled()}};

"use strict";C3.Behaviors.Physics.Acts={ApplyForce(a,b,c){this._ApplyForce(a,b,c)},ApplyForceToward(a,b,c,d){this._ApplyForceToward(a,b,c,d)},ApplyForceAtAngle(b,c,a){this._ApplyForceAtAngle(b,C3.toRadians(c),a)},ApplyImpulse(a,b,c){this._ApplyImpulse(a,b,c)},ApplyImpulseToward(a,b,c,d){this._ApplyImpulseToward(a,b,c,d)},ApplyImpulseAtAngle(b,c,a){this._ApplyImpulseAtAngle(b,C3.toRadians(c),a)},ApplyTorque(a){this._ApplyTorque(C3.toRadians(a))},ApplyTorqueToAngle(b,c){this._ApplyTorqueToAngle(C3.toRadians(b),C3.toRadians(c))},ApplyTorqueToPosition(a,b,c){this._ApplyTorqueToPosition(C3.toRadians(a),b,c)},SetAngularVelocity(a){this._SetAngularVelocity(C3.toRadians(a))},CreateDistanceJoint(a,b,c,d,e){if(b){const f=b.GetFirstPicked(this._inst);this._CreateDistanceJoint(a,f,c,d,e)}},CreateRevoluteJoint(a,b){if(b){const c=b.GetFirstPicked(this._inst);this._CreateRevoluteJoint(a,c)}},CreateLimitedRevoluteJoint(a,b,c,d){if(b){const e=b.GetFirstPicked(this._inst);this._CreateLimitedRevoluteJoint(a,e,C3.toRadians(c),C3.toRadians(d))}},CreatePrismaticJoint(a,b,c,d,e,f,g,h,i){if(b){const j=b.GetFirstPicked(this._inst);this._CreatePrismaticJoint(a,j,C3.toRadians(c),d,e,f,g,C3.toRadians(h),i)}},RemoveJoints(){this._RemoveJoints()},SetWorldGravity(a){this.GetBehavior().SetGravity(a)},SetSteppingMode(a){this.GetBehavior().SetSteppingMode(a)},SetIterations(a,b){this.GetBehavior().SetIterations(a,b)},SetVelocity(a,b){this._SetVelocity(a,b)},SetDensity(a){this._SetDensity(a)},SetFriction(a){this._SetFriction(a)},SetElasticity(a){this._SetElasticity(a)},SetLinearDamping(a){this._SetLinearDamping(a)},SetAngularDamping(a){this._SetAngularDamping(a)},SetImmovable(a){this._SetImmovable(a)},EnableCollisions(a,b){this.GetBehavior().SetCollisionsEnabled(this.GetObjectClass(),a,0!==b)},SetPreventRotate(a){this._SetPreventRotate(0!==a)},SetBullet(a){this._SetBullet(0!==a)},SetEnabled(a){this._SetEnabled(0!==a)}};

"use strict";C3.Behaviors.Physics.Exps={VelocityX(){return this._GetVelocityX()},VelocityY(){return this._GetVelocityY()},AngularVelocity(){return C3.toDegrees(this._GetAngularVelocity())},Mass(){return this._GetMass()},CenterOfMassX(){return this._GetCenterOfMassX()},CenterOfMassY(){return this._GetCenterOfMassY()},Density(){return this._GetDensity()},Friction(){return this._GetFriction()},Elasticity(){return this._GetElasticity()},LinearDamping(){return this._GetLinearDamping()},AngularDamping(){return this._GetAngularDamping()}};
...
self.C3_GetObjectRefTable = function () {
	return [
		...
		C3.Plugins.Spriter.Acts.SetPos,
		C3.Plugins.Sprite.Exps.X,
		C3.Plugins.Sprite.Exps.Y,
		C3.Behaviors.Pin.Acts.Pin,
		C3.Plugins.Spriter.Acts.ZMoveToObject,
		C3.Plugins.Spriter.Acts.setPlaybackSpeedRatio,
		...
		C3.Plugins.Spriter.Acts.resumeAnimation,
		C3.Plugins.Spriter.Cnds.IsBoolInstanceVarSet,
		C3.Plugins.Sprite.Cnds.OnCollision,
		C3.Plugins.Spriter.Acts.ToggleBoolInstanceVar,
		...
		C3.ScriptsInEvents.Main_Event22_Act1,
		C3.Plugins.Spriter.Acts.setAnimationLoop,
		...
		C3.ScriptsInEvents.Main_Event27_Act1,
...
self.C3_JsPropNameTable = [
	...
	{initialized: 0},
	{objectTouched: 0},
	{xVelocityIncrement: 0},
	{yVelocityIncrement: 0},
	{bubbleUID: 0},
	{objectUID: 0},
	{objectName: 0},
	{objectsLeft: 0},
...

{
	const scriptsInEvents = {
		...
		{
			main__bubble_tapped(runtime.objects.Bubble.getFirstPickedInstance());
		},
...
(function () {
	...
	globalThis.main__init = async () => {
		...
		if(!initialized) {
			...
			objectsArray = mainJsonResponse.objects;
