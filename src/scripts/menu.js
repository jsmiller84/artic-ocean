class Menu {
  ...
  async init() {
    ...
    if (!this.initialized) {
      const menuJsonURL = await this.runtime.assets.getProjectFileUrl("Menu.json");
      const menuJsonResponse = await this.runtime.assets.fetchJson(menuJsonURL);
  ...
  async setPage(pageNumber) {
    ...
    this.runtime.objects.ItemText.getAllInstances().forEach((text) => {
      text.destroy();
    });

    this.runtime.objects.Toggle.getAllInstances().forEach((text) => {
      text.destroy();
    });

    this.runtime.objects.SoundBorder.getAllInstances().forEach((border) => {
      border.destroy();
    });
    ...
    for (let colNdx = 0; colNdx < currentPage.columns.length; ++colNdx) {
      ...
      for (let ndx = 0; ndx < soundList.length; ++ndx) {
        ...
        const topBorder = await this.runtime.objects.SoundBorder.createInstance(1, currX + 100, topY);
        topBorder.height = 10;
        topBorder.width = 900;

        const leftBorder = await this.runtime.objects.SoundBorder.createInstance(1, currX + 100, topY);
        leftBorder.height = (currY - topY) + 40;
        leftBorder.width = 10;

        const rightBorder = await this.runtime.objects.SoundBorder.createInstance(1, (currX + 100) + 890, topY);
        rightBorder.height = (currY - topY) + 40;
        rightBorder.width = 10;

        const bottomBorder = await this.runtime.objects.SoundBorder.createInstance(1, (currX + 100), currY + 40);
  ...
  async setupSelectedSounds() {
    this.runtime.objects.SelectedText.getAllInstances().forEach((text) => {
      text.destroy();
    });
    ...
    const topBorder = await this.runtime.objects.SoundBorder.createInstance(0, xPos + 100, yPos);
    topBorder.height = 10;
    topBorder.width = 700;

    const bottomBorder = await this.runtime.objects.SoundBorder.createInstance(0, xPos + 100, yPos + 1300);
    bottomBorder.height = 10;
    bottomBorder.width = 700;

    const leftBorder = await this.runtime.objects.SoundBorder.createInstance(0, xPos + 100, yPos);
    leftBorder.height = 1300;
    leftBorder.width = 10;

    const rightBorder = await this.runtime.objects.SoundBorder.createInstance(0, xPos + 790, yPos);
  ...
  async createText(text, x, y, size, width, height) {
    const soundObject = await this.runtime.objects.ItemText.createInstance(0, x, y);
  ...
  async createSelectedText(text, x, y, size, width) {
    const soundObject = await this.runtime.objects.SelectedText.createInstance(0, x, y);
  ...
  async createToggle(object, x, y, width, height) {
    const toggle = await this.runtime.objects.Toggle.createInstance(0, x, y);
    toggle.width = width;
  ...
  async toggleButton(button) {
    ...
    if (uidArray) {
      uidArray.forEach((uid) => {
        const placementObject = this.runtime.getInstanceByUid(uid);
...
(function () {
  ...
  globalThis.menu__select_sounds = async () => {
    globalThis.main__loadSounds = menu.selectedList;
    menu.runtime.goToLayout("Main");
  }
  ...
  globalThis.menu__toggle_button = async (button) => {
    await menu.toggleButton(button);
  }

  globalThis.menu__toggle_placement = async (text) => {
    await menu.togglePlacement(text);
  }
